 ...
 */
class Animal {
    constructor(name, species, age, birthday, isDomestic) {
        this.name = name;
...
/* Task 1 - Part 2. Replace these three instances of the example Class Resource with instances
   of your own class */
/*
model.add(new Animal("One"));
model.add(new Animal("Two"));
model.add(new Animal("Three"));
*/
model.add(new Animal("Luna", "Cat", 3, new Date("2022-05-01").toISOString().slice(0, 10), false));
model.add(new Animal("Max", "Dog", 5, new Date("2021-11-12").toISOString().slice(0, 10), true));
model.add(new Animal("Coco", "Parrot", 2, new Date("2023-02-18").toISOString().slice(0, 10), true));
