...
function add(resource, sibling) {
    ...
    creator.append(new ElementCreator("p").text("Species: " + resource.species))
    creator.append(new ElementCreator("p").text("Age: " + resource.age))
    creator.append(new ElementCreator("p").text("Birthday: " + resource.birthday))
    creator.append(new ElementCreator("p").text("Domestic: " + resource.isDomestic))
    ...
    creator
        ...
        .append(new ElementCreator("button").text("Remove").listener('click', () => {
            ...
            xhr.open('DELETE', `/api/resources/${resource.id}`);
            xhr.onload = () => {
                if (xhr.status === 204) {
...
function edit(resource) {
    ...
    */
    ...
    formCreator
        ...
        .append(new ElementCreator("label").text("Birthday").with("for", "resource-birthday"))
        .append(new ElementCreator("input").id("resource-birthday").with("type", "date").with("value", new Date(resource.birthday).toISOString().slice(0, 10)))
        .append(new ElementCreator("label").text("Domestic").with("for", "resource-domestic"))
        .append(resource.isDomestic
            ? new ElementCreator("input").id("resource-domestic").with("type", "checkbox").with("checked", "")
            : new ElementCreator("input").id("resource-domestic").with("type", "checkbox")
        );
//.append(new ElementCreator("input").id("resource-domestic").with("type", "checkbox"));
    ...
    formCreator
        .append(new ElementCreator("button").text("Speichern").listener('click', (event) => {
            ...
            //The default action of the browser for a form button is submit so we tell the browser that the form is handled explicitly
            ...
            */
            ...
            resource.birthday = document.getElementById("resource-birthday").value;
            resource.isDomestic = document.getElementById("resource-domestic").checked;
            ...
            */
            const xhr = new XMLHttpRequest();
            xhr.open('PUT', `/api/resources/${resource.id}`);
...
function create() {
    const formCreator = new ElementCreator("form")
        .id("create-form")
        .append(new ElementCreator("h3").text("Add Animal"))
        .append(new ElementCreator("label").text("Name").with("for", "resource-name"))
        ...
        .append(new ElementCreator("label").text("Birthday").with("for", "resource-birthday"))
        .append(new ElementCreator("input").id("resource-birthday").with("type", "date"))
        .append(new ElementCreator("label").text("Domestic").with("for", "resource-domestic"))
        .append(new ElementCreator("input").id("resource-domestic").with("type", "checkbox"))
        .append(new ElementCreator("button").text("Speichern").listener('click', (event) => {
            event.preventDefault();
            ...
            const newAnimal = {
                ...
                species: document.getElementById("resource-species").value,
                age: document.getElementById("resource-age").value,
                birthday: document.getElementById("resource-birthday").value,
                isDomestic: document.getElementById("resource-domestic").checked
            };
            ...
            xhr.onload = () => {
                if (xhr.status === 200) {
                    add(Object.assign(new Animal(), JSON.parse(xhr.responseText)));
    ...
    const parent = document.querySelector('main');
    formCreator.insertBefore(parent, document.querySelector('#bottom'));
}
